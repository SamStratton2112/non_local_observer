...
const NavigationBar = () => {
  return (
    <Navbar collapseOnSelect expand="md" className="NavBar navbar-static-top NavBarBckground" data-bs-theme="dark">
      <Navbar.Brand as={Link} to="/" className='nav-name NavBar-link'>
        Non Local Observer
      </Navbar.Brand>
      <Nav className="ms-auto">
        <Navbar.Toggle aria-controls="responsive-navbar-nav me-2"/>
        <Navbar.Collapse id="responsive-navbar-nav">
          <Nav.Link className='NavBar-link'  as={Link} to="/shop">Shop</Nav.Link>
          <Nav.Link className='NavBar-link'  as={Link} to="/cart">Cart</Nav.Link>
          <Nav.Link className='NavBar-link' as={Link} to="/contact">Contact</Nav.Link>
          <Nav.Link className='NavBar-link'  as={Link} to="/">About Zoe</Nav.Link>
